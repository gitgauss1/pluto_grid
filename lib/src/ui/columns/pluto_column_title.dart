...
class _ColumnWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DragTarget<PlutoColumn>(
      onWillAcceptWithDetails: (DragTargetDetails<PlutoColumn> columnDetails) {
        return columnDetails.data.key != column.key &&
            !stateManager.limitMoveColumn(
              column: columnDetails.data,
              targetColumn: column,
            );
      },
      onAcceptWithDetails: (DragTargetDetails<PlutoColumn> columnDetails) {
        if (columnDetails.data.key != column.key) {
          stateManager.moveColumn(column: columnDetails.data, targetColumn: column);
        }
      },
